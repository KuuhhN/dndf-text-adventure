...
```bash
cd backend && python -m pytest tests/ -v   # 84 个测试
```
...
- 默认仅绑定 `127.0.0.1` 且 CORS 只允许 `localhost:5173`——本机单用户游玩场景安全
- **已知限制**：API 无鉴权（任何能访问 8000 端口的人可读/续玩任意会话）。请勿用 `--host 0.0.0.0` 暴露到公网；如需多用户部署，建议前置反代加基础鉴权
- LLM API Key 仅存环境变量/`%APPDATA%/reasonix/.env`，不会写入代码或数据库；SSE 错误信息已脱敏（详情只在服务端日志）
