...
1. 玩家行动或场景接触需要判定成败的内容时（潜行/攀爬/说服/察觉/开锁/追踪/搜索/威吓/哄骗，
   以及解读历史文献、魔法符文、宗教圣物、药草毒物、神秘线索等**专业信息**），
   必须调用 ability_check 工具，**并传 dc（场景难度：普通 10 / 困难 15 / 极难 20）**，
   由引擎判定成败（返回 success）。成功才给予详细情报/资源，失败只能给模糊印象或一无所获；
   角色接触专业信息时应**自动发起检定**，即使玩家未主动提出。严禁直接叙述成败或编造检定结果。
...
5. 叙事中出现任务/悬赏/委托（告示板、委托信、NPC 委托等）时，必须调用 post_quest 工具
   把任务注册到告示栏（title/description/reward/status）。玩家接下任务后，用
   post_quest(status="accepted") 更新状态。
6. 玩家获得/拾取/购买/搜刮到物品时，必须调用 add_item 入背包；消耗/使用/丢弃/交出物品时
   必须调用 remove_item。物品数量以引擎状态为准，严禁在叙事中自行增减。
7. 遇到有名字或重要互动的 NPC（酒馆老板、委托者、敌人首领、神秘陌生人等）时，必须调用
   register_npc 记录（身份/位置/关系/备注）。再次遇到同一 NPC 时也要调用更新关系与位置。
...
def build_system_prompt(character: dict, lore_hits: list[dict] | None = None) -> str:
    ...
    # 当前位置与可达区域注入（LLM 叙事地点必须与地图一致——防「王都下楼去村庄酒馆」类脱节）
    loc_key = character.get("location") or "tavern"
    loc = tools.WORLD_MAP.get(loc_key, {})
    if loc:
        nbs = []
        for nb in loc.get("neighbors", []):
            n = tools.WORLD_MAP.get(nb, {})
            nbs.append(f"{n.get('name', nb)}（{nb}）")
        base += ("\n\n## 当前位置\n"
